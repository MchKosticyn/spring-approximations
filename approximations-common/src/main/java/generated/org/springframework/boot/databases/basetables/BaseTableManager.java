...
public class BaseTableManager<T, V> implements ITableManager<T> {
    ...
    public void initialize(DTOInfo<T, V> dtoInfo) {
        ...
        String[] idFieldsNames = dtoInfo.getIdFieldsNames();
        String[] relatedFieldsNames = dtoInfo.getRelatedFieldsNames();
        this.baseTable = new BaseTable<>(entityType, isAutoGenerateId, idFieldsNames, relatedFieldsNames);
        this.tablesChain = this.baseTable;

        if (!isAutoGenerateId) applyIdValidation();
        if (SpringDatabases.validator != null)
            applyFieldsValidation(fieldsToValidateNames, baseTable.getUserEntitiesSize());

        if (dtoInfo.isNeedTrack()) applyTrack(tableName, entityType, baseTable.getUserEntitiesSize());

        // DTO functions
        this.baseTable.setDTOFunctions(
                dtoInfo.getBlankInit(),
                dtoInfo.getRelationsInit(),
                dtoInfo.getRelationsInitForConcrete(),
    ...
    private void applyFieldsValidation(String[] fieldsToValidateNames, int userEntitiesSize) {
        this.tablesChain = new BaseTableConstraintValidate<>(this.tablesChain, fieldsToValidateNames, userEntitiesSize);
    }

    private void applyTrack(String tableName, Class<T> entityType, int userEntitiesSize) {
        this.tablesChain = new BaseTableTrack<>(this.tablesChain, tableName, entityType, userEntitiesSize);
    }
    ...
    // all of this should be concrete
    public CacheTable<T> getConcreteEntities(Collection<T> references) {
        checkInitialized();
        List<Object[]> referencesKeys = new ArrayList<>();
        for (T ref: references)
            referencesKeys.add(buildId(ref));

        Function<T, Boolean> filter = (T t) -> {
            Object[] id = buildId(t);
            for (Object[] referenceKey : referencesKeys) {
                boolean found = true;
                for (int i = 0; i < id.length; i++) {
                    if (!id[i].equals(referenceKey[i])) {
                        found = false;
                        break;
                    }
                }
        ...
        FiltredTable<T> filtred = new FiltredTable<>(baseTable.getConcreteEntities(), filter);
        return new CacheTable<>(filtred);
    }

    // all of this should be concrete
    public T getConcreteEntity(T reference) {
        checkInitialized();
        Object[] refId = buildId(reference);

        Function<T, Boolean> filter = (T t) -> {
            Object[] id = buildId(t);
            for (int i = 0; i < id.length; i++) {
                if (!id[i].equals(refId[i])) return false;
            }
